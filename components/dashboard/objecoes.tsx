  ...
  },
  {
    ...
    contexto: (
      <>
        ...
        <strong className="font-semibold text-slate-900">suporte humano, especializado e disponível diariamente</strong>.
      </>
    ),
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <h1 className="mt-3 text-3xl font-bold tracking-tight text-slate-900">Objeções</h1>
        <p className="mt-2 text-base text-slate-600">Como responder às dúvidas mais comuns</p>
        <p className="mt-4 max-w-2xl text-sm leading-relaxed text-slate-600">
          ...
          <strong className="font-semibold text-slate-900">demonstração sem compromisso com um consultor da Catedral</strong>.
        </p>
      </div>

      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <h2 className="text-2xl font-bold text-slate-900">Se o cliente disser...</h2>
        <div className="mt-6 space-y-6">
          {objecoes.map((obj) => (
            <div key={obj.cliente} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-5">
              <div className="flex items-start gap-4">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-900 text-white">
                  <obj.icon className="h-5 w-5" />
                </div>
                <div className="flex-1">
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">O cliente diz</p>
                  <h3 className="mt-0.5 text-base font-semibold text-slate-900">&ldquo;{obj.cliente}&rdquo;</h3>
                  <p className="mt-2 text-sm leading-relaxed text-slate-600">{obj.contexto}</p>

                  <div className="mt-3 rounded-xl border-l-4 border-emerald-400 bg-emerald-50 px-4 py-3">
                    <p className="text-xs font-semibold uppercase tracking-wide text-emerald-800">Você pode responder</p>
                    <p className="mt-1 text-sm italic text-emerald-900">&ldquo;{obj.resposta}&rdquo;</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-bold text-slate-900">Como lidar com uma objeção</h2>
        </div>
        <p className="mt-3 text-sm text-slate-600">Procure seguir três princípios simples:</p>
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          {principios.map((principio, index) => (
            <div key={principio.title} className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-sm font-bold text-white">
                {index + 1}
              </div>
              <h3 className="mt-3 text-sm font-semibold text-slate-900">{principio.title}</h3>
              <p className="mt-1.5 text-sm text-slate-600">{principio.body}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-3 rounded-xl border-l-4 border-slate-300 bg-white px-4 py-3">
        ...
        <p className="mt-3 text-sm text-slate-600">
          ...
          <strong className="font-semibold text-slate-900">Indicar Cliente</strong> no Portal do
          Parceiro.
        </p>
        <div className="mt-5">
          <Link
